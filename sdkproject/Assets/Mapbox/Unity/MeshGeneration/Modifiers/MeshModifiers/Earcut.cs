...
{
	public class EarcutLibrary
	{
		public List<int> Earcut(List<float> data, List<int> holeIndices, int dim)
		{
			dim = Math.Max(dim, 2);
			...
			earcutLinked(outerNode, triangles, dim, minX, minY, size, 1);

			return triangles;
		}

		private void earcutLinked(Node ear, List<int> triangles, int dim, float minX, float minY, float size, int pass = 0)
		{
			if (ear == null) return;
		...
		private bool isEarHashed(Node ear, float minX, float minY, float size)
		{
			var a = ear.prev;
			var b = ear;
			var c = ear.next;
		...
		private int zOrder(float x, float y, float minX, float minY, float size)
		{
		...
		private void splitEarcut(Node start, List<int> triangles, int dim, float minX, float minY, float size)
		{
			var a = start;
			do
			{
		...
		//private bool isEarHashed(Node ear, int minX, int minY, int size)
		//{
		//	var a = ear.prev;
		//	var b = ear;
		//	var c = ear.next;
		...
		private void indexCurve(Node start, float minX, float minY, float size)
		{
			var p = start;
			do
			{
				if (p.mZOrder == null) p.mZOrder = zOrder(p.x, p.y, minX, minY, size);
				p.prevZ = p.prev;
				p.nextZ = p.next;
				p = p.next;
			} while (p != start);
		...
		private Node EliminateHoles(List<float> data, List<int> holeIndices, Node outerNode, int dim)
		{
			var i = 0;
		...
		public Data Flatten(List<List<float[]>> data)
		{
		...
		public Data Flatten(List<List<Vector3>> data)
		{
			var dim = 2;
			var result = new Data() { Dim = dim };
			var holeIndex = 0;
